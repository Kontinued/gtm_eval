...
if st.button("Run pipeline"):
    ...
    for draft, evaluation in rounds:
        round_tokens = draft.tokens + evaluation.tokens
        st.markdown(
            f"**Round {draft.round}** · generated by: {draft.source} · "
            f"{round_tokens:,} tokens")
    ...
    total_tokens = sum(d.tokens + e.tokens for d, e in rounds)
    st.metric("Tokens used (generation + judge)", f"{total_tokens:,}")
    st.caption(
        "Mock drafts report an estimate; live drafts and the LLM judge report "
        "real usage from the API. This is the number the cost ceiling caps."
    )
